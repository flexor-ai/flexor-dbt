{% macro categories_statistics(flex_table, reference_table=null, reference_table_fields=null, filter_nulls=true) %}
...
WITH filtered_data AS (
    SELECT
        ...
        {{ flexor.category('flex_table.flex') }} as category
        {% if reference_table_fields and reference_table_fields|length > 0 %}
    ...
    {% endif %}
    {% if filter_nulls %}
    WHERE {{ flexor.category('flex') }} is not null
    {% endif %}
),
PredictionCounts AS (
    SELECT
